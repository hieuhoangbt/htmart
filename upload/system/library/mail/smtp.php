...
class Smtp {
	...
	public $smtp_port = 25;
	public $smtp_method = 'ssl';
	public $smtp_timeout = 5;
	public $verp = false;

	function overwriteSendUsingPHPMailer()
    {
        $mail = new PHPMailer;

        $mail->isSMTP();                                      // Set mailer to use SMTP
        $mail->Host = $this->smtp_hostname;  // Specify main and backup SMTP servers
        $mail->SMTPAuth = true;                               // Enable SMTP authentication
        $mail->Username = $this->smtp_username;                 // SMTP username
        $mail->Password = $this->smtp_password;                           // SMTP password
        $mail->SMTPSecure = ($this->smtp_method);                            // Enable TLS encryption, `ssl` also accepted
        ...
        $mail->addReplyTo($this->reply_to, 'Admin');
	...
	public function send() {
	    return $this->overwriteSendUsingPHPMailer();
		if (is_array($this->to)) {
			$to = implode(',', $this->to);
		} else {
			$to = $this->to;
